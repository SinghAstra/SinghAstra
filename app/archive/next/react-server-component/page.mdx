Your Unit 1 syllabus is a solid foundation, but based on our recent technical "deep dive," it needs to be updated to better reflect the structural and transport mechanics we uncovered (specifically the RSC Payload and the Hybrid Hydration logic).

I have rewritten Unit 1 into three specific parts to avoid overwhelming the document and to ensure we capture the "Iterative Deepening" detail you require.

---

# Unit 1: React Server Components (RSC) & The Payload

## Part 1: The Hydration Problem & The RSC Solution

### 1.1 The Hydration Problem: The "JavaScript Tax"

In traditional Client-Side Rendering (CSR) and standard SSR, the browser is burdened by the Hydration Problem. Even if the server sends HTML, the browser must:
...
The Failure Point: As applications grow, the "Main Thread" of the browser becomes blocked by JavaScript execution. This results in high Total Blocking Time (TBT) and poor Interaction to Next Paint (INP). The user sees the UI but cannot interact with it.

### 1.2 The RSC Solution: Zero Bundle Size

React Server Components (RSC) shift the "work" from the browser to the server. By executing components on the server and sending only the Result, we achieve:
...
## Part 2: The Transport Mechanics (The RSC Payload)

### 1.3 Technical Anatomy of the RSC Payload
...
## Part 3: Component Taxonomy & Hydration

### 1.4 The Server Component (Default)
...
### 1.5 The Client Component (`'use client'`)
...
### Next Step: Unit 2 - Rendering Strategies

Now that we understand the payload, we need to understand timing. When does the server generate this payload? At Build-time (Static) or Request-time (Dynamic)?

Would you like me to generate the detailed notes for Unit 2 next?
