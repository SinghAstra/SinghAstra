...
1. Immutable: Once created, we cannot change this object. If we want to update the UI (e.g., change `className` to active), we must create a brand new object. We cannot scribble on the old ticket.
2. Transient: These objects are created, compared, and thrown away (Garbage Collected) in milliseconds.

> Key Insight: React is okay with creating and destroying 10,000 of these simple objects per second. Modern JavaScript engines (V8 in Chrome) are incredibly fast at handling short-lived objects. The performance cost of React usually comes from the calculation inside our components, not the object creation itself.
...
5. It touches the Real DOM only where necessary (`Reconciliation`).
...
_This is the fundamental reason why we need `useMemo` and `useCallback` — to force React to keep the object the same between runs._

Here are the answers to your deep-dive questions.

### _When does the conversion happen from JSX to Blueprint Object ?_

You are distinguishing between "Code Transformation" and "Code Execution."

- Build Time (On our computer/server):
  This is where Babel works. It takes our JSX file (`<div />`) and rewrites the text into standard JavaScript text (`React.createElement('div')`). The browser never sees JSX. It receives a bundle file containing thousands of `React.createElement` function calls.
...
Yes, standard JavaScript objects are mutable. We could technically modify a property on an object in the heap.
