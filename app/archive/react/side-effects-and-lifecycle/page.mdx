...
## 5. Lifecycle Comparison: Class vs. Hooks
...
### _2. If we have `useEffect(() => { console.log(count) }, [])` (empty array), and `count` changes from 0 to 5, what will the console log? 0 or 5?_

It will log 0.

- Why: The effect ran only once when the component mounted. At that exact moment in time, `count` was `0`.
- The Closure: The function inside `useEffect` closed over the variable `count` from the first render. It never ran again, so it never saw the updated value. It is holding onto a stale version of the variable.
- The Fix: If we want to see the live value, `count` must be in the dependency array: `[count]`.

### _3. In React 18 Strict Mode (development), `useEffect` runs twice on mount. Why does React do this intentionally?_

React is stress-testing our Cleanup Function.

- The Sequence: React does: `Mount` → `Unmount` → `Mount`.
