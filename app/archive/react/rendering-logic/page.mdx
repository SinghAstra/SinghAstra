...
- Warning: Be careful with numbers. If `unreadMessages.length` is `0`, JavaScript considers `0` as "falsy" but still renders the number `0`.
- Fix: Ensure the left side is a boolean: `unreadMessages.length > 0 && ...`
...
- Why: This is a classic JavaScript "gotcha." The `&&` operator returns the value of the first falsy operand it finds. Since `0` is falsy, the expression short-circuits and returns `0`. React renders numbers, so you see a "0" printed on your UI.
- The Fix: Always coerce the condition to a true boolean.
