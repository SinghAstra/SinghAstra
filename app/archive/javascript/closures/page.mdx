...
A Closure is formed when an inner function returned from outer function is able to access and modify the variable defined in outer function even after function is popped off Call Stack.
...
2.  The Promotion: The engine realizes variable accessed in inner function cannot live in the temporary Stack Frame. It allocates a specific Context Object (Closure) in the Heap and stores referenced variable there.
3.  The Link: The returned inner function has its `[[Environment]]` pointer linked to this Heap Object, not the dead Stack Frame.
4.  Result: inner function acts like a Backpack. It carries the function logic plus a reference to the Heap data it needs.
