...
In many languages (like Java or C++), functions are distinct structures compiled differently than variables. In JavaScript, functions are `First-Class Citizens`.
...
#### B. Internal Architecture (The 3 Hidden Slots)
...
1.  `[[Call]]`: The code logic. This slot is triggered when you invoke the function with `()`.
2.  `[[Construct]]`: The factory logic. This slot is triggered only when you invoke the function with the `new` keyword (creates a new instance).
3.  `[[Environment]]`: The memory link. A pointer to the Scope where the function was created. In case of closures, [[Environment]] is the specific pointer that links that function to the Context Object in the Heap.
...
#### A. The Lightweight Architecture

Arrow functions are designed to be non-constructible.

- Missing Slot: They DO NOT have the `[[Construct]]` slot.
- The Consequence: You cannot use `new` with an Arrow Function. If you try `new Arrow()`, the engine throws a `TypeError` because the internal factory logic simply doesn't exist.
...
- Limitation: You cannot use array methods like `.map()` or `.filter()` on it.
