...
The Rule: An `async` function always returns a Promise.
...
It removes the callback structure entirely. We read the code top-to-bottom, just like we read synchronous code, even though time gaps exist between the lines.
...
### 4. The Serial Trap (Performance)

This is the most common mistake senior developers look for in code reviews.

Because `await` pauses execution, using it inside a loop or sequentially can accidentally kill performance by forcing tasks to run one by one (Serially) instead of at the same time (Parallel).
...
Answer:
It is Non-Blocking for the Browser (Event Loop), but Blocking for the code inside the function.
