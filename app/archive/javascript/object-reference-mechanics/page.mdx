...
To understand how data moves in your application, you must master the `Assignment Rule`. When you assign a variable to another variable (`let a = b`), the engine does not look inside the data. It blindly copies the Stack Slot.

#### A. Primitives (Copy by Value)

When you assign a Primitive, the engine copies the actual value.
...
When you assign an Object, the engine copies the `Memory Address` (the pointer), not the Heap data.
...
Aliasing is dangerous because it might lead to unintended result.You can modify a variable in File A, and it accidentally updates a variable in File B because they share the same Heap Address.

#### A. Mutation vs. Reassignment

This distinction is critical for React and Functional Programming.
...
- Reality: `const` creates an Immutable Binding. It locks the Stack Slot. You cannot Reassign the variable to a new address.
- Loophole: It does not lock the Heap. You are free to Mutate the contents of the object referenced by that slot.
