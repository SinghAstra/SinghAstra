...
In the previous archive, we learned that standard Objects have a fatal flaw: they implicitly convert all keys to Strings. ES6 introduced proper data structures to solve this architectural limitation.
...
- No Coercion: You can use an Object, a Function, or a Number as a key. The engine does not convert them to strings.
...
A Set is a special collection type—a set of values (without keys)—where each value may occur only once.

- Uniqueness: If you try to add the same value twice, the Set automatically ignores the duplicate.
