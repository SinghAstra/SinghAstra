...
Unlike standard variables, which are determined by where you write the code (Lexical/ Static Scope), `this` has Dynamic Scoping. It is determined entirely by how the function is called (Invocation).

---

In case of defined with function declaration:

### The 4 Rules of Invocation

#### 1. New Binding (The Constructor)
...
This is the most common rule. If the function is called as a method of an object (it has a dot `.` before it), `this` points to the object on the left side of the dot.
...
#### 3. Default Binding (The Catch-All)
...
#### 4. Explicit Binding (The Overrides)

This rule applies when you want to force a function to use a specific object as its context, overriding the Implicit or Default rules. We use three built-in methods which is inherited by all function objects.
...
- Mechanism: Invokes the function immediately.
- Arguments: Passed as a single Array. Useful when you have a list of data.
