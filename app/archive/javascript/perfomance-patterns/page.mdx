...
### 1. The Problem: Event Floods

Browsers are hyper-responsive. They fire events for every micro-interaction.

- `window.onresize`: Fires constantly as you drag the window edge.
- `document.onscroll`: Fires on every pixel of movement.
- `input.onkeyup`: Fires on every single character typed.

The Crash Scenario:
If we attach an expensive operation (like an API call `fetch` or a DOM re-render) directly to these events, the Call Stack gets flooded. The browser cannot keep up, frames drop, and the UI lags.

We need a way to control the flow.
...
function debounce(func, delay) {
  ...
  return function (...args) {
    ...
    // 2. Set a new timer
    timer = setTimeout(() => {
      func.apply(this, args); // Run the actual logic
    }, delay);
  };
}
...
### _Stop and Think: Why did we use `func.apply(this, args)` in the examples above instead of just `func(args)`? _

Answer:
To preserve the Context (`this`).
If we attach the debounced function to an object method (e.g., `user.save()`), calling it normally might cause `this` to point to the `Window` or `undefined` (depending on strict mode). Using `.apply(this)` ensures that the `this` from the event handler is passed correctly to your original function.
